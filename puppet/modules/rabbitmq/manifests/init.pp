...
class rabbitmq(
  ...
  $external_repos             = $rabbitmq::params::external_repos,
  ...
  $key_content                = undef,
  $myvhost                    = $rabbitmq::params::myvhost,
  $config_perms               = $rabbitmq::params::config_perms,
  ...
  if $vhostuser != undef {
    rabbitmq_user { $vhostuser:
      admin    => true,
      password => 'sensu',
    }
    rabbitmq_user_permissions { "$vhostuser@$myvhost":
